...
contract Payroll is Ownable{
    ...
    function addEmployee(address empid,uint salary) onlyOwner employeeNotExist(empid){
        employees[empid] = Employee(empid, salary.mul(1 ether), now);
        totalSalary = totalSalary.add(employees[empid].salary);
        
    }
    
    function removeEmployee(address empid) onlyOwner employeeExist(empid){
 		var employee = employees[empid];
    ...
    function updateEmplpyee (address empid,uint salary)onlyOwner employeeExist(empid){
    	var employee = employees[empid];
      	_partialPay(employee);
      	totalSalary = totalSalary.sub(employees[empid].salary);
   ...
   function calculateRunway() public  returns (uint times){
